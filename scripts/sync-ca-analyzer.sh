...
# Source repo (entra-ca-scanner) — override by passing the web/ dir as $1
ANALYZER_WEB="${1:-$HOME/Desktop/Coreview2/Entra_Security_Scanner_for_App_Registrations/web}"
...
if [[ ! -d "$ANALYZER_WEB" ]]; then
  ...
  echo "  Pass it explicitly:  $0 /path/to/entra-ca-scanner/web" >&2
  exit 1
fi
